...
export function BottomNav({ activeView, onViewChange }: BottomNavProps) {
  ...
  return (
    <nav className={cn(
      "fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md z-50 backdrop-blur-2xl border-t h-20 pb-safe px-2 flex justify-around items-center transition-colors duration-300",
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={cn(
              'flex flex-col items-center gap-1 transition-all duration-300 relative px-2.5 py-2 rounded-full sm:px-4',
              btnClass
            )}
          >
